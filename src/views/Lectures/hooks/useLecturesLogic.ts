...
export const useLecturesLogic = () => {
  const intl = useIntl();
  const [lectureView, setLectureView] = useState<LectureType>(
    LectureType.MyLectures
  );
  ...
  const {
    ...
    error: lecturerError,
  } = useLecturerByUserQuery({
    userId: userId,
    enabled: role === Role.Lecturer,
  });
  ...
  const semesterMap = useMemo(
    () =>
      Object.fromEntries(
        semesters?.map((semester) => [semester.id, semester.name]) ?? []
      ),
    [semesters]
  );
  ...
  const userLectures = useMemo(() => {
    ...
    return (
      lectures?.filter((lecture) => lectureIds?.includes(lecture.id)) ?? []
    );
  ...
  const getFilteredFormValues = useCallback(
    (questionType: QuestionType) => {
      ...
      return formValues.filter(
        (formValue) =>
          questionTypeMap.get(formValue.questionID) === questionType
      );
    },
    ...
    (questionType: QuestionType) => {
      ...
      const groupedByLecture =
        filteredFormValues.reduce(
          (acc, formValue) => {
            const lectureID =
              courseAssignmentToLectureMap[formValue.courseAssignmentID];
            if (!lectureID) {
              console.warn(
  ...
  useEffect(() => {
    setFilteredLectures(
      lectureView === LectureType.MyLectures ? userLectures : lectures
    );
  ...
  const buttons = useMemo(
    () => [
      {
        value: LectureType.MyLectures,
        label: intl.formatMessage(translations.my),
      },
      {
        value: LectureType.AllLectures,
        label: intl.formatMessage(translations.all),
      },
    ],
    [intl]
  );
