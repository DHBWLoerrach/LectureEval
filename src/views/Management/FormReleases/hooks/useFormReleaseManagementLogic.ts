...
export const useFormReleaseManagementLogic = () => {
  ...
  const { mutate: upsertCourseAssignment } =
    useUpsertCourseAssignmentMutation();
  const { mutate: deleteCourseAssignment } =
  ...
  useEffect(() => {
    if (!assignmentsError && !formsError && !departmentsError && !coursesError)
      return;
